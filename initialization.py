...
"""
import AMCA_Direct as amcaD
import AMCA_Starlet as amcaS
import numpy as np
...
def GMCAperpatch(Starlet,X, dS, dPatch,  aMCA=1, Init=1,t_start=0):
    ...
    if Starlet:
        print('Starlet')
        Aref1,Sref1=amcaS.amcaps(X, dS, dPatch, aMCA=aMCA, Init=Init)
    else:
        print('direct')
        Aref1,Sref1=amcaD.amcaps(X, dS, dPatch, aMCA=aMCA, Init=Init)
    ...
    for j in range(n_patches):
        ...
        dS1['t']=Patchsize
        if Starlet:
            a,s=amcaS.amcaps(Xdir, dS1, dPatch, aMCA=aMCA, Init=Init)
        else:
            a,s=amcaD.amcaps(Xdir, dS1, dPatch, aMCA=aMCA, Init=Init)
